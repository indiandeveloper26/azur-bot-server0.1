...
const TELEGRAM_BOT_TOKEN = 'YOUR_TELEGRAM_BOT_TOKEN';
const CHAT_ID = 'YOUR_CHAT_ID'; // आपका Telegram User ID या Group ID
...
// Active trades का sample data - आप इसे DB से लोड कर सकते हैं
let activeTrades = [
  {
    symbol: 'ETHUSDT',
    entry: 2479.23,
...
async function analyzeSymbol(symbol) {
  const candles1m = await getCandles(symbol, INTERVAL_1M, 50);
  const candles1h = await getCandles(symbol, INTERVAL_1H, 50);

  const closes1m = candles1m.map(c => c.close);
  const volumes = candles1m.map(c => c.volume);

  const rsi1m = RSI.calculate({ values: closes1m, period: 14 }).slice(-1)[0];
  const ema14 = EMA.calculate({ values: closes1m, period: 14 }).slice(-1)[0];
  ...
  const avgVolume = SMA.calculate({ values: volumes, period: 20 }).slice(-1)[0];
  const ema200 = EMA.calculate({ values: candles1h.map(c => c.close), period: 200 }).slice(-1)[0];
  ...
  if (rsi1m > 50 && currentPrice > ema14 && macd.MACD > macd.signal && volumes.slice(-1)[0] > avgVolume && currentPrice > ema200) {
    finalSignal = 'BUY';
    target = currentPrice * 1.004;
    stoploss = currentPrice * 0.998;
  } else if (rsi1m < 50 && currentPrice < ema14 && macd.MACD < macd.signal && volumes.slice(-1)[0] > avgVolume && currentPrice < ema200) {
    finalSignal = 'SELL';
    target = currentPrice * 0.996;
...
function formatSignalMessage(data) {
  const {
    symbol, currentPrice, rsi1m, ema14, macd1m, macdSignal1m,
    volume, avgVolume, ema200, finalSignal, target, stoploss
  } = data;

  const signalEmoji = finalSignal === 'BUY' ? '🟢 BUY' : finalSignal === 'SELL' ? '🔴 SELL' : '⚪ HOLD';

  return `📡 Crypto Signal - ${symbol} (${INTERVAL_1M} + ${INTERVAL_1H})
📈 Signal: ${signalEmoji}
💰 Entry: ${currentPrice.toFixed(2)}
🎯 Target: ${target.toFixed(2)}
🛑 Stoploss: ${stoploss.toFixed(2)}

📊 RSI (${INTERVAL_1M}): ${rsi1m.toFixed(2)}
📉 EMA14 (${INTERVAL_1M}): ${ema14.toFixed(2)}
📈 MACD (${INTERVAL_1M}): ${macd1m.toFixed(2)}
🟡 Signal Line: ${macdSignal1m.toFixed(2)}
📊 Volume: ${volume}
📉 Avg Vol SMA20: ${avgVolume.toFixed(0)}
📉 EMA200 (${INTERVAL_1H}): ${ema200.toFixed(2)}`;
}

// Format Active Trades Message
function formatActiveTrades(trades) {
  if (trades.length === 0) return '🚫 No active trades currently.';

  let msg = '🔔 Active Trades Status:\n\n';
  trades.forEach((t, i) => {
    msg += `${i + 1}. ${t.symbol}\n`;
    msg += `   Signal: ${t.signal}\n`;
    msg += `   Entry: ${t.entry}\n`;
    msg += `   Target: ${t.target}\n`;
    msg += `   Stoploss: ${t.stoploss}\n\n`;
  });
  return msg;
}
...
async function sendSignal(chatId) {
  ...
  if (!data || data.finalSignal === 'HOLD') {
    await bot.sendMessage(chatId, 'No clear BUY/SELL signal now.');
    return;
  }
  ...
  await bot.sendMessage(chatId, signalMessage, opts);
}
...
bot.on('callback_query', async query => {
  ...
  if (query.data === 'active_trade_status') {
    const msg = formatActiveTrades(activeTrades);
    await bot.sendMessage(chatId, msg);
    await bot.answerCallbackQuery(query.id); // remove loading circle
  }
});

// Start bot & send initial signal
bot.on('message', async (msg) => {
  if (msg.text === '/start' || msg.text === '/signal') {
    await sendSignal(msg.chat.id);
  }
});
